...
//        xfermode = new PorterDuffXfermode(PorterDuff.Mode.SRC_OUT);
    ...
    private void initAttribute(Context context, AttributeSet attrs) {
        TypedArray t = context.obtainStyledAttributes(attrs, R.styleable.PwdInputView);

        maxLength = t.getInt(R.styleable.PwdInputView_pwv_maxLength, 6);
        borderColor = t.getColor(R.styleable.PwdInputView_pwv_borderColor, Color.GRAY);
        pwdColor = t.getColor(R.styleable.PwdInputView_pwv_pwdColor, Color.GRAY);
        inputBorderColor = t.getColor(R.styleable.PwdInputView_pwv_haveInputBorderColor, borderColor);
        asterisk = t.getString(R.styleable.PwdInputView_pwv_asterisk);

        if (asterisk == null || asterisk.length() == 0) asterisk = "*";
        else if (asterisk.length() > 1) this.asterisk = asterisk.substring(0, 1);

        radius = t.getDimensionPixelSize(R.styleable.PwdInputView_pwv_radius, 0);
        strokeWidth = t.getDimensionPixelSize(R.styleable.PwdInputView_pwv_strokeWidth, 0);
        spacing = t.getDimensionPixelSize(R.styleable.PwdInputView_pwv_spacing, 0);
        borderStyle = t.getInt(R.styleable.PwdInputView_pwv_borderStyle, BorderStyle.BOX);
        pwdStyle = t.getInt(R.styleable.PwdInputView_pwv_pwdStyle, PwdStyle.CIRCLE);
...
////        super.onDraw(canvas); // 去掉EditText默认的绘制
    ...
    private void drawPassword(Canvas canvas, int index) {
        ...
        switch (pwdStyle) {
            case PwdStyle.CIRCLE: // 绘制圆点，radius可以改变圆点大小
                canvas.drawCircle((rectF.left + rectF.right) / 2, (rectF.top + rectF.bottom) / 2, 18, paint);
                break;
